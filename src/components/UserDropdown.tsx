...
interface RoleSwitcherProps {
  ...
  onRoleChange: (newRole: 'student' | 'teacher') => void
}
...
function RoleSwitcher({ currentRole, onRoleChange }: RoleSwitcherProps) {
  ...
  return (
    <button
      onClick={() => onRoleChange(isTeacher ? 'student' : 'teacher')}
...
export default function UserDropdown({
  ...
  const handleRoleSwitch = async (newRole: 'student' | 'teacher') => {
    ...
    try {
      ...
      // Update player preferences with new role
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {showDropdown && (
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-100 py-2 z-50">
          ...
          {/* Role Switcher */}
          <RoleSwitcher
            currentRole={player?.player_preferences?.current_role || 'student'}
            onRoleChange={async (newRole) => {
              setShowDropdown(false)
              await handleRoleSwitch(newRole)
            }}
          />
