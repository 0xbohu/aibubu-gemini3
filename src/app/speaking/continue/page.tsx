    ...
    const getUser = async () => {
      ...
      if (languagesData) {
        ...
        if (selectedLanguageCode) {
          ...
          // Check if user has a level for this language
          if (targetLanguage && playerData?.player_levels?.speaking) {
            const hasLevel = playerData.player_levels.speaking[selectedLanguageCode]
            
            // If no level set for this language, redirect to assessment
            if (!hasLevel) {
              router.push(`/assessment/speaking/${selectedLanguageCode}`)
              return
            }
          }
                ...
                {currentLevel ? (
                  ...
                  <div className="mb-6">
                    <div className="bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-4 mb-4">
                      <p className="text-yellow-800 font-bold text-sm">⚠️ No level set for this language</p>
                      <p className="text-yellow-700 text-xs mt-1">Set your level to get personalized lessons</p>
                    </div>
                  </div>
                )}
