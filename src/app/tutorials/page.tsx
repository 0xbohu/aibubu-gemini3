...
export default function TutorialsPage() {
  ...
  const categoryInfo = {
    ...
    thinking: { title: 'Thinking & Logic', emoji: '🧠', color: 'purple' },
    science: { title: 'Science', emoji: '🔬', color: 'green' },
    reading: { title: 'Reading', emoji: '📖', color: 'blue' },
    writing: { title: 'Writing', emoji: '✏️', color: 'indigo' }
  }
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      // Get tutorials based on category (all types)
      let query = supabase
        .from('tutorials')
        .select('*')
  ...
  useEffect(() => {
    ...
    const filtered = tutorials.filter(tutorial => {
      ...
      if (tutorial.tutorial_type === 'speaking') {
        ...
        // If no level set for this language, show all tutorials
        if (!languageLevel) return true
        
        // Extract the db level (numeric difficulty) from the language level object
        const dbLevel = typeof languageLevel === 'object' ? languageLevel.dbLevel : null
        
        // If no valid db level, show all tutorials
        if (!dbLevel) return true
        
        // Show tutorials at user's level or higher (more challenging)
        return tutorial.difficulty_level >= dbLevel
      }
      ...
      if (tutorial.tutorial_type === 'maths') {
        ...
        // If no level set, show all tutorials
        if (!mathLevel) return true
        
        // Extract the db level (numeric difficulty) from the math level object
        const dbLevel = typeof mathLevel === 'object' ? mathLevel.dbLevel : null
